import{_ as n}from"./plugin-vue_export-helper-DlAUqK2U.js";import{c as a,o as s,b as e}from"./app-BiHadoIp.js";const i={},l=e(`<h2 id="应用和包管理" tabindex="-1"><a class="header-anchor" href="#应用和包管理"><span>应用和包管理</span></a></h2><p>涉及复杂的实现逻辑和复杂多样的需求，一次性实现所有功能几乎是不可能的。为了解决这个问题，高级编程语言都提供一套模块化组织代码的方法，以便将复杂问题拆解成 多个相对简单的子问题，分别实现对应功能，再组合到一起。</p><p>库(library)：实现了某个功能，但又不直接体现在最终实现的功能里的代码模块。</p><p>档案文件(archive file)：将一个库或者应用包含的所有源代码文件以及元数据文件(元数据指描述对象一般特征的数据，例如名称、版本、作者、依赖等信息，保存这些数据的文件 就叫元数据文件)压缩放在一个文件中，这个文件叫档案文件。</p><p>包(package)：一个或者多个档案文件组合在一起，方便 人们分享、查找和维护称之为包。</p><p>软件源(software source/software repository)：提供这些包的服务器成为软件源。</p><p>包管理器(package manager)：方便不熟悉操作系统底层的用户提供查找、安装、卸载、升级软件等操作的管理器。</p><h3 id="使用apt管理应用" tabindex="-1"><a class="header-anchor" href="#使用apt管理应用"><span>使用<code>apt</code>管理应用</span></a></h3><p>常用命令</p><div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre class="language-bash"><code><span class="token comment"># 列出系统已安装应用</span>
